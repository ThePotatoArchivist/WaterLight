...
public class BiomeColorsMixin {
	...
	private static int interpolate(int original, @Local(argsOnly = true)BlockRenderView world, @Local(argsOnly = true)BlockPos pos) {
		...
		return ColorHelper.Argb.lerp(WaterLight.getSaturateProgress(lightLevel == 0 && WaterLight.SODIUM_INSTALLED ? world.getLightLevel(LightType.SKY, pos.up()) : lightLevel), WaterLight.NO_LIGHT_WATER_COLOR, original);
	}
}